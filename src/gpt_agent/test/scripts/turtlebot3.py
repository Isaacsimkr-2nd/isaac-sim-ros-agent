...
class gptAgent(Node):
    def __init__(self):
        super().__init__('gpt_agent_tools')
        ...
        self.image_path = '/tmp/gpt_image.jpg'
    ...
    def publish_twist_to_cmd_vel(self, velocity: float, angle: float, duration: float = 1.0) -> str:
        ...
        self.move_flag = True
        return f"gpt 이동 명령: velocity={velocity}, angle={angle}, duration={duration}s."

    def stop_gpt(self) -> str:
        self.move_flag = False
        self.stop_movement()
        return "gpt 즉시 정지 명령 실행됨."
...
def get_gpt_agent():
    global gpt_agent, ros_thread
    if gpt_agent is None:
        rclpy.init(args=None)
        gpt_agent = gptAgent()
        def ros_spin():
            rclpy.spin(gpt_agent)
...
def stop_gpt() -> str:
    ...
    :return: 정지 명령 결과"""
    agent = get_gpt_agent()
    return agent.stop_gpt()

@tool
def get_gpt_position() -> str:
    """gpt의 현재 위치를 반환합니다.
    :return: x, y, yaw 값"""
    agent = get_gpt_agent()
